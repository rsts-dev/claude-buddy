...
async function createDefaultConfiguration(targetDirectory, manifest, installedComponents, environment, transaction, verbose) {
  ...
  const buddyConfigPath = path.join(targetDirectory, '.claude-buddy', 'buddy-config.json');
  const buddyConfig = {
    version: manifest.version,
    personas: {
      autoActivation: true,
    ...
    hooks: {
      enabled: installedComponents.includes('hooks'),
      timeout: 10000
    },
    templates: {
      defaultFoundation: 'default'
    }
  };
  ...
  try {
    ...
    logger.error(`Failed to create buddy-config.json: ${error.message}`, verbose);
    throw error;
  }

  // Create hooks.json if hooks are installed
  if (installedComponents.includes('hooks')) {
    ...
    try {
      ...
      logger.error(`Failed to create hooks.json: ${error.message}`, verbose);
    }
  }
}
